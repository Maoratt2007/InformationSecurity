...
export async function ensureRegistrationKeyBundleUploaded({
  ...
  const storedBundle = tryLoadBundleFromSessionStorage(userId);
...
function tryLoadBundleFromSessionStorage(userId: string) {
  try {
    return KeyStorageService.load(userId);
  } catch (error) {
    console.warn("[KeyStorageService] Could not load private keys from sessionStorage.", error);
    return undefined;
  }
}

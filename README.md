...
1. Clear `localStorage` on both, refresh, and **log in again** (restores the local private key bundle).
...
If you only cleared `localStorage` without logging in again, X3DH cannot run (no private bundle).
